...
//        生成实例时，对项目进行初始化
    ...
    {
//        这里的几个都是用的参数包裹类，放到对象的$this->parameters进行存储，后面会用到。
        $this->request = new ParameterBag($request);
        $this->query = new ParameterBag($query);
        $this->attributes = new ParameterBag($attributes);
        $this->cookies = new ParameterBag($cookies);
//        文件的参数的设置相对而言会稍微复杂一些，FileBag中的fixPhpFilesArray，用来修复$_FILES中存在的一个bug，不过殊途同归，最终也会设置到$this->parameters中
        $this->files = new FileBag($files);
        //server数组主要是$_SERVER的参数，他没有自身的构造方法，所以直接存到了$this->parameters中
        $this->server = new ServerBag($server);
        //这里主要是设置一些请求头信息，数据存放在$this->headers中
    ...
    public static function createFromGlobals()
    {
        ...
        $server = $_SERVER;
//        PHP_SAPI这是php的内置常量，用来检测是在client下还是浏览器执行的，cli是命令行，cli-server是浏览器
