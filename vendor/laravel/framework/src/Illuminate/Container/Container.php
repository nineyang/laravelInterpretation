...
//        同时删除$this->instances和$this->aliases里面的的key
...
//        如果这个不是一个回调函数的话,就生成一个回调函数,这个回调函数需要注入一个容器作为参数
    ...
    {
//        调用当前对象的removeAbstractAlias方法,此方法主要是用于判断$this->aliases是否已经包含，如果没有，返回，如果有，删除$this->abstractAlias里面的对应元素
...
//        添加到$this->instances数组中
        $this->instances[$abstract] = $instance;
//        调用bound方法判断是否$this->instances或者$this->bindings数组，或者$this->aliases数组有设置这个key,如果有在其中一个数组中设置,那么就调用rebound方法
        if ($this->bound($abstract)) {
            $this->rebound($abstract);
        }
    }
...
//        如果绑定的二者一样，或者说concrete是一个回调，执行build
        ...
        //这里主要是用来设置share的，也就是singleton和bind的区别，如果share是true的话，就会放在$this->instances中，下次就可以直接使用
