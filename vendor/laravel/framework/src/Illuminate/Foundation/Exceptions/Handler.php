...
//            $this->convertExceptionToResponse($e)返回一个response对象
    ...
    {
//        根据exception的信息来配置FlattenException的信息，并返回一个静态实例
        $e = FlattenException::create($e);

//        生成一个ExceptionHandler
        $handler = new SymfonyExceptionHandler(config('app.debug', false));

//        getHtml主要是根据不同的status和debug输出不同的内容，最后返回一个response对象
...
//            重新生成一个Http/Response的实例
            $response = new Response($response->getContent(), $response->getStatusCode(), $response->headers->all());
        }

//        返回生成的response对象，不过会在$this->exception注入$e
